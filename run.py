...
def load_config(config_path: str = "./configs/config.yaml") -> Dict[str, Any]:
    default_cfg = {
        ...
        "decision": {"alpha_ip": 0.5, "beta_cos": 0.3, "gamma_overlap": 0.2,
                     "threshold": 0.7, "top_refs": 3},
        "evidence": {"not_found_sim_threshold": 0.15, "insertion_rate_percent": 0.1, "dedup_round_digits": 3},
...
def _append_round_log(csv_path: str, row_dict: dict):
    header = ["round","gated","not_found","inserted","packs_total_after_rebuild"]
...
class FeedNidsOrchestrator:
    ...
    async def handle_predictions_csv(self, predictions_csv: str, round_name: str, out_dir: str) -> dict:
        ...
        for ep in packs:
            ...
            d = self.policy.decide(alert_id=ep["alert_id"], candidates=res.get("candidates", []))
            decisions.append(d)
        ...
        # 5) 미검색 → Evidence DB 저장 (Normal 우선 없음)
        not_found = self.writer.select_not_found(packs, rag_map)
        ...
        # 6) (선택) adjusted_label 반영 파일 생성
        if "alert_id" in df_pred.columns:
            dec_df = pd.DataFrame(decisions)
            # 결정→라벨 매핑(정상/공격 자동화 신호만 반영)
            def _map_adj(decision: str):
                if decision in ["AUTO_SUPPRESS","AUTO_NORMALIZE","AUTO_CONFIRM_NORMAL"]:
                    return "Normal"
                if decision in ["AUTO_CONFIRM_ATTACK"]:
                    return "Attack"
                return None
            dec_df["adjusted_label"] = dec_df["decision"].map(_map_adj)
            df_pred_out = df_pred.merge(dec_df[["alert_id","adjusted_label"]], on="alert_id", how="left")
            df_pred_out.to_csv(os.path.join(out_dir, "predictions_with_adjusted.csv"), index=False)

        # 요약
        summary = {"round": round_name, "gated": int(len(packs)),
                   "not_found": int(len(not_found)), "inserted": int(len(saved_ids))}
        with open(os.path.join(out_dir, "round_summary.json"), "w", encoding="utf-8") as f:
            f.write(json.dumps(summary, ensure_ascii=False, indent=2))
        print(f"[{round_name}] decisions={len(decisions)} | not_found={len(not_found)} | inserted={len(saved_ids)}")
        return {"decisions": decisions, "inserted": len(saved_ids), "gated": len(packs), "not_found": len(not_found)}
...
async def main():
    ...
    rag = FeedbackSearchTool(
        ...
        index_type=R.get("index_type", "hnsw"),
        hnsw_M=R.get("hnsw", {}).get("M", 32),
        hnsw_efSearch=R.get("hnsw", {}).get("efSearch", 128),
        ivf_nlist=R.get("ivf_pq", {}).get("nlist", 8192),
        ivf_nprobe=R.get("ivf_pq", {}).get("nprobe", 32),
        pq_m=R.get("ivf_pq", {}).get("pq_m", 64),
        pq_bits=R.get("ivf_pq", {}).get("pq_bits", 8),
    ...
    pol = DecisionPolicy(alpha_ip=D.get("alpha_ip", 0.5), beta_cos=D.get("beta_cos", 0.3),
                         gamma_overlap=D.get("gamma_overlap", 0.2),
                         threshold=D.get("threshold", 0.7), top_refs=D.get("top_refs", 3))
    writer = EvidenceWriter(db_dir=evidence_db_dir,
                            ...
                            insertion_rate_percent=E.get("insertion_rate_percent", 0.1),
